...
class User{
	...
	function consulUser($ini, $fin){
		$conexao = getConexao();
		$query = "SELECT u.ID, u.USER, u.NOME, u.EMAIL, t.TIPO FROM TBUSER AS u, TBTIPO AS t WHERE u.TID = t.ID ORDER BY u.NOME LIMIT $ini, $fin ";
	...
	function checarUser(){
    	$conexao = getConexao();
		$query = "SELECT ID FROM TBUSER WHERE USER='".$this->login."' AND PASS='".$this->passw."'";
    	$resultados = mysql_query($query) or die(mysql_error());
    	mysql_close($conexao);
