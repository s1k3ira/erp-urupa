...
class Cupom{
	...
	function cadCupom(){
		$sql= sprintf("INSERT INTO TBCUPOM(CID,VID,PID,UID,CUP_DATE,CUP_DATV,CUP_DATP,CUP_HORA,CUP_CLIE,CUP_CNPC,CUP_MOTO,CUP_PLAC,CUP_ESTA,CUP_QUAN,CUP_PUNI,CUP_PRON,CUP_VALO,CUP_DESC,CUP_TOTAL,CUP_USER,CUP_SITU)VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
	...
	function cupCons($dati, $datf, $scon){
		$conexao = getConexao();
		$query = "SELECT v.CID, SUM(v.QUANT) as TOQUANT, SUM(v.CUP_DESC) as TODESC, SUM(v.CUP_TOTAL) as TOTOTAL, c.CLI_NOME, c.CLI_CNPJ, c.CLI_CPF FROM TBCUPOM AS v, TBCLIENTE as c WHERE v.CUP_SITU='".$scon."' AND v.CUP_DATE >= '".$dati."' AND v.CUP_DATE <= '".$datf."' AND v.CID = c.ID GROUP BY v.CID ORDER BY c.CLI_NOME";  
