...
//Pega Cliente no Banco
	...
	$cli = new Cupom();
	if($_GET[tip] == v){
		$resul = $cli->cupConsV($dati, $datf, $scon);
	}else{
...
<script>
	function listarDias(){
		...
		location.href='cupAbe.php?dati='+diai+'&datf='+diaf+'&sit=<?= $_GET[sit]?>&tip=<?= $_GET[tip]?>';
	}
</script>
...
?>
  <tr bgcolor="<?= $cor?>">
        ...
        <td align="center"><a href="cupChe.php?cid=<?= $cli["CID"]?>&dati=<?= $_GET[dati]?>&datf=<?= $_GET[datf]?>&sit=<?= $scon?>&tip=<?= $_GET[tip]?>"><img src="pictures/b_search.png" border="0" /></a></td>
  </tr>
<?
}
?>
